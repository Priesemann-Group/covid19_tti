...
Rtcmax_e0 = fzero(@(Rtk) maxVp(Pools(xidef,nudef,Rtk,Gamma,lambda_s,lambda_r,0,epsilondef)),2); %eta
...
for j = [1 2 3 4 5]
    ...
    hold on
    plot([Rtcmax Rtcmax],[0 0.1],'r--','LineWidth',fact_curva,'HandleVisibility','off')
    hold on
    plot([Rtcmax_e0 Rtcmax_e0],[0 0.1],'r--','LineWidth',fact_curva,'HandleVisibility','off')
    hold on
    xlim([0.75 3])
    ylim([0 0.15])
    %extractData(0.75,3,0,15,strcat('Rev1_FigS2b_',num{j},'.csv'))
