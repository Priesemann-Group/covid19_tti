...
str1 = {'Rev1_FigS3a.csv';'Rev1_FigS3c.csv';'Rev1_FigS3e.csv';'Rev1_FigS3g.csv'};
str2 = {'Rev1_FigS3b.csv';'Rev1_FigS3d.csv';'Rev1_FigS3f.csv';'Rev1_FigS3h.csv'};
...
loc = 'northeast';
PhiNmax = 3546;
PhiMax = PhiNmax*[0.5 1 1.5 2];

sigma = 2;
nmax = 300;
eta = 0.66;

for k = 1:length(PhiMax)
    close all
    Phimax = PhiMax(k);
    ...
    [Tsol,x] = ode45(@(t,x) Pools_solver_phit(t,x,xi,nu,Rt,Gamma,lambda_s,lambda_r,eta,epsilon,nmax,Phi,Phimax,sigma),t,x0);
    x = x';
    N_hat = newInfections(t,x,Gamma,nu,Rt,lambda_s,lambda_r,nmax,eta);
    N_sum = newInfections_Total(t,x,Gamma,nu,epsilon,Rt,PHI(t,0,sigma,Phi,Phimax));
    ...
    ylim([0 ylimsupI])
    %extractData(tminplot,tmax,0,ylimsupI,str1{k})
    hold on
    ...
    hold on
    ylim([0 ylimsupN])
    %extractData(tminplot,tmax,0,ylimsupN,str2{k})
    ax.TickLabelInterpreter='latex';
