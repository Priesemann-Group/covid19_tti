...
for k = 1:kmax
    for i = 1:length(lambda_r)
        try
            eta(k,i) = fzero(@(et) maxVp(Pools(xi,nu,Rt(k),Gamma,lambda_s,lambda_r(i),et,epsilon)),0);
        catch err
            disp('oops')
        end
    end
...
for i = 1:kmax
    a = 0.5*(i-1)/(kmax-1);
    plot(eta(i,:),lambda_r,'Color',(1-a)*red+a*blue,'LineWidth',3*fact_curva);
    str{1,i} = char(strcat("$R^H\!=",num2str(Rt(i)),"$"));
    hold on
end
ylim([0 lmax])
xlim([0 1])
%extractData(0,lmax,0,1,'Rev1_Fig7a.csv')
...
xlabel('$\eta$','interpreter','latex','FontSize',siz*fact_label)
